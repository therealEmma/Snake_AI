...
def update_direction():
    ...
    if new_direction not in [Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT]:
        return jsonify({"success": False, "error": "Invalid direction"}), 400
    # Update the snake's direction (assuming current_game.direction is a string or enum)
    if hasattr(current_game, 'direction'):
        if hasattr(current_game.direction, 'name'):
            current_game.direction = getattr(Direction, new_direction)
        else:
            current_game.direction = new_direction
    else:
        # Fallback if current_game.direction is not an attribute (e.g. a property or computed)
        # (In a real implementation, you might need to call a setter or update a state variable.)
         pass
    return jsonify({"success": True, "message": "Direction updated"})
